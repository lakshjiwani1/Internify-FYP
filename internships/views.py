...
def create_internship(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Ensure the 'company' attribute is set before saving
            if internship.company:
                internship.save()
                message.append("Internship Saved Successfully")
                return redirect(internship_list)
            else:
                # Handle the case where 'company' is not set
                message.append("Error: Company is not set.")
        else:
            # Handle the case where the form is not valid
            message.append("Error: Form is not valid.")
    else:
...
def delete_internship(request, pk):
    ...
    if internship.company == request.user.companyauth:
        ...
        internship.delete()
        messages.success(request, f"Internship Deleted: {internship_title}")
    else:
